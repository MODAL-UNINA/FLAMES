...
def divide_images_and_masks(dataset_name, path, number_img_server_test, split,
    ...
    for f in files_img:
        ...
        if number_img_server_test != 0:
            ...
            remaining_data = {k: v for k, v in list(patient_data.items()) if k not in [t[0] for t in test_server_list]}
...
if dict_name == f'{dataset_name}_split.pkl':
    ...
    number_img_server_test = 65

n_samples = divide_images_and_masks(dataset_name, path, number_img_server_test, split,
                                    output_dir, dict_name)
